...
The separate **Self-hosted three-carrier probe** workflow runs only when manually dispatched on `main` with repository variable `CLASH_RELAY_CARRIER_PROBE_ENABLED=true`. It first reuses the complete `ci.yml` gate and receives its `validated_sha`. Every carrier job and the collector check out that exact commit and verify that `GITHUB_SHA`, checkout `HEAD`, and `validated_sha` match before installing or running repository code. A failed CI gate prevents the secret-bearing probe jobs from starting. Register three Linux self-hosted runners with dedicated labels `carrier-probe-telecom`, `carrier-probe-unicom`, and `carrier-probe-mobile`, attached to their respective carrier networks. Configure branch-restricted, protected environments `carrier-probe-telecom`, `carrier-probe-unicom`, and `carrier-probe-mobile` for the corresponding jobs, with access to `CLASH_RELAY_SUBSCRIPTIONS` and a repository-specific secret `CLASH_RELAY_CARRIER_HMAC_KEY`; the three jobs must use the same key and subscription input. The workflow scheduler's `runs-on` labels and those protected environments are the runner and secret trust boundary. Application environment variables, including any caller-supplied label string, are not independent proof of runner identity. GitHub-hosted runners never run the carrier producer. Without this infrastructure, carrier evidence stays `not_configured` and normal production publishing continues independently.
