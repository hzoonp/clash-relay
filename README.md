...
Carrier observation history is a separate aggregate-only Cloudflare KV state (`<production-key>.carrier-observation-history-v1`), never scheduler or AI cache state. Only candidate-bound campaigns with full coverage, current freshness, and sufficient evidence update each carrier's reachable-ratio, latency, and outcome-ratio EMAs. Other campaigns may increment status counters but do not update quality. The state uses a 30-day rolling window capped at 64 campaign timestamps and publishes numeric counts and EMAs without `improving`, `stable`, or `degrading` verdicts. Aggregate-only evidence cannot rank individual nodes by carrier; any future node-level carrier ranking requires a separate privacy-preserving per-node evidence contract.
