...
The separate **Self-hosted three-carrier probe** workflow runs only when manually dispatched on `main` with repository variable `CLASH_RELAY_CARRIER_PROBE_ENABLED=true`. It first reuses the complete `ci.yml` gate and receives its `validated_sha`. Every carrier job, the collector, and the history commit job check out that exact commit and verify that `GITHUB_SHA`, checkout `HEAD`, and `validated_sha` match before installing or running repository code. A failed CI gate prevents the secret-bearing probe jobs from starting. Register three Linux self-hosted runners with dedicated labels `carrier-probe-telecom`, `carrier-probe-unicom`, and `carrier-probe-mobile`, attached to their respective carrier networks. Configure branch-restricted, protected environments `carrier-probe-telecom`, `carrier-probe-unicom`, and `carrier-probe-mobile` for the corresponding jobs. Also configure a branch-restricted, protected `carrier-probe-collector` environment for the GitHub-hosted collector and history commit job. Put the same `CLASH_RELAY_SUBSCRIPTIONS` input and repository-specific `CLASH_RELAY_CARRIER_HMAC_KEY` secret in all four environments. The workflow scheduler's `runs-on` labels and protected environments are the runner and secret trust boundary. Application environment variables, including any caller-supplied label string, are not independent proof of runner identity. The GitHub-hosted collector validates and ingests aggregate evidence; GitHub-hosted runners never run the carrier producer. Without this infrastructure, carrier evidence stays `not_configured` and normal production publishing continues independently. The carrier workflow uses `clash-relay-carrier-probe-${{ github.ref }}` to limit duplicate campaigns, while normal publication uses `clash-relay-publish-${{ github.ref }}`. After successful preflight, only the separate commit job holds `clash-relay-carrier-history-${{ github.ref }}` while writing history KV; the privacy-safe receipt travels as an encoded job output, never as an uploaded artifact. Carrier runner queues and history commits cannot hold the production publication lock.

Each runner builds the private candidate and probes the same deterministic bounded endpoint sample. TCP-native endpoints use DNS resolution and bounded TCP connection attempts; UDP-native endpoints are counted as skipped. Raw candidates, targets, endpoint details, credentials, and probe samples stay on the private runner. Only anonymous aggregate JSON crosses the job boundary. The collector checks all three aggregates, then feeds `carrier-qualification.json` into the canonical production preflight. The preflight regenerates the production candidate and recomputes its anonymous sample, inventory, and probe-plan IDs with the environment-only HMAC key; any candidate or key drift rejects the evidence. The key is required only when carrier input is supplied. The preflight step is strictly read-only even though it receives Cloudflare credentials for reading production state: a fully successful preflight writes a local privacy-safe observation receipt in `.work/carrier-observation-receipt.json`, binding the schema version, aggregate digest, candidate/probe binding digest, collected epoch, and validated SHA. A separate explicit commit step then runs `scripts/persist_carrier_observation.py` with that receipt, authenticates its repository-keyed receipt MAC and re-verifies the digests and validated SHA, and fails closed without writing history on any mismatch. Ordinary dry-runs produce no committable receipt. If the preflight fails, the job stops before the commit step and nothing is written. The workflow deletes its local aggregate and receipt after the commit attempt, uploads no probe artifact, and does not publish configuration. Carrier evidence remains `external_self_hosted_advisory`; client URLTest retains local authority.

Sampling uses repository-bound HMAC identities for endpoint deduplication, the selected sample set, the full eligible inventory, and the selected probe plan. It balances canonical geographic regions, protocol, and subscription source with a cap of 12 TCP targets; `ANY` and `CHAIN` are non-geographic, and conflicting geographic labels for one endpoint fail closed. The collector requires matching sample, inventory, and plan IDs, sampled counts, sampler versions, and timestamps within five minutes. Coverage (`partial`/`full`) is separate from evidence quality (`sufficient`/`insufficient`/`stale`): sufficiency requires fully tested distinct TCP endpoints and geographic or source diversity, with a bounded fallback for small inventories. Protocol labels remain observability only. Aggregate-only diversity and failure-category counts (`dns_failure`, `connect_timeout`, `connection_refused`, `connect_failure`, `tcp_connected`) document results without exposing identities. UDP-only endpoints are counted as skipped; an endpoint also sampled through TCP is not counted as skipped. Evidence remains advisory and never prunes nodes or changes Promotion Guard or scheduling.

Carrier observation history is a separate aggregate-only Cloudflare KV state (`<production-key>.carrier-observation-history-v1`, document schema v3), never scheduler or AI cache state. Only candidate-bound campaigns issued through a validated observation receipt can enter the state, so `binding_failed` is not a persistable status: persisted statuses are `valid`, `stale`, `partial`, `insufficient`, and `invalid` (reserved for a bound campaign with inconsistent aggregate semantics). Only bound campaigns with full coverage, current freshness, and sufficient evidence update each carrier's reachable-ratio, latency, and outcome-ratio EMAs. `recent_campaign_count` counts campaigns inside the trailing 30-day window and is reaped on every write and read. A privacy-safe `campaign_id` derived from the signed receipt aggregate digest makes retries idempotent; older receipts leave EMA unchanged, while distinct campaigns collected in the same second are processed in serialized commit order. `status_counts_lifetime` and each carrier's `campaign_runs_lifetime` are saturating lifetime counters capped at 64 — they are not rolling 30-day values; a carrier row resets only after 30 days without a bound valid campaign. Latency evidence carries its own freshness fields: `latency_sample_runs` counts campaigns with reachable samples and `last_latency_epoch` records when latency was last measured, so a zero-reachable campaign never makes an old latency EMA read as current evidence or as latency 0. The state publishes numeric counts and EMAs without `improving`, `stable`, or `degrading` verdicts. Aggregate-only evidence cannot rank individual nodes by carrier; any future node-level carrier ranking requires a separate privacy-preserving per-node evidence contract.
