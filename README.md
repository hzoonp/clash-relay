...
The separate **Self-hosted three-carrier probe** workflow runs only when manually dispatched on `main` with repository variable `CLASH_RELAY_CARRIER_PROBE_ENABLED=true`. It first reuses the complete `ci.yml` gate and receives its `validated_sha`. Every carrier job and the collector check out that exact commit and verify that `GITHUB_SHA`, checkout `HEAD`, and `validated_sha` match before installing or running repository code. A failed CI gate prevents the secret-bearing probe jobs from starting. Register three Linux self-hosted runners with dedicated labels `carrier-probe-telecom`, `carrier-probe-unicom`, and `carrier-probe-mobile`, attached to their respective carrier networks. Configure branch-restricted, protected environments `carrier-probe-telecom`, `carrier-probe-unicom`, and `carrier-probe-mobile` for the corresponding jobs. Also configure a branch-restricted, protected `carrier-probe-collector` environment for the GitHub-hosted collector. Put the same `CLASH_RELAY_SUBSCRIPTIONS` input and repository-specific `CLASH_RELAY_CARRIER_HMAC_KEY` secret in all four environments. The workflow scheduler's `runs-on` labels and protected environments are the runner and secret trust boundary. Application environment variables, including any caller-supplied label string, are not independent proof of runner identity. The GitHub-hosted collector validates and ingests aggregate evidence; GitHub-hosted runners never run the carrier producer. Without this infrastructure, carrier evidence stays `not_configured` and normal production publishing continues independently.
...
Carrier observation history is a separate aggregate-only Cloudflare KV state (`<production-key>.carrier-observation-history-v1`, document schema v2), never scheduler or AI cache state. Only candidate-bound campaigns issued through a validated observation receipt can enter the state, so `binding_failed` is not a persistable status: persisted statuses are `valid`, `stale`, `partial`, `insufficient`, and `invalid` (reserved for a bound campaign with inconsistent aggregate semantics). Only bound campaigns with full coverage, current freshness, and sufficient evidence update each carrier's reachable-ratio, latency, and outcome-ratio EMAs. `recent_campaign_count` counts campaigns inside the trailing 30-day window and is reaped on every write and read. `status_counts_lifetime` and each carrier's `campaign_runs_lifetime` are saturating lifetime counters capped at 64 — they are not rolling 30-day values; a carrier row resets only after 30 days without a bound valid campaign. Latency evidence carries its own freshness fields: `latency_sample_runs` counts campaigns with reachable samples and `last_latency_epoch` records when latency was last measured, so a zero-reachable campaign never makes an old latency EMA read as current evidence or as latency 0. The state publishes numeric counts and EMAs without `improving`, `stable`, or `degrading` verdicts. Aggregate-only evidence cannot rank individual nodes by carrier; any future node-level carrier ranking requires a separate privacy-preserving per-node evidence contract.
