...
Carrier observation history 使用独立的 aggregate-only Cloudflare KV 状态（`<production-key>.carrier-observation-history-v1`，文档 schema v2），与 scheduler history、AI cache 完全隔离。只有通过校验 observation receipt 提交、candidate binding 通过的 campaign 才能进入该状态，因此 `binding_failed` 不是可持久化状态；可持久化状态为 `valid`、`stale`、`partial`、`insufficient` 与 `invalid`（仅限已绑定但聚合语义异常的安全情况）。只有 coverage 为 full、freshness 为 current、evidence 为 sufficient 的 campaign 才更新各运营商可达率、延迟和结果类别的 EMA。`recent_campaign_count` 统计最近 30 天窗口内的 campaign，并在每次写入与读取时回收过期条目。`status_counts_lifetime` 与每个运营商的 `campaign_runs_lifetime` 是上限 64 的饱和 lifetime 计数器，不是滚动 30 天值；运营商行仅在该运营商连续 30 天没有 bound valid campaign 后重置。延迟证据自带新鲜度字段：`latency_sample_runs` 统计有可达样本的 campaign 数，`last_latency_epoch` 记录最近一次测到延迟的 epoch，因此 reachable=0 的 campaign 既不会让旧延迟 EMA 看似本轮证据，也不会被伪造成 latency 0。状态仅输出数值计数和 EMA，不生成 `improving`、`stable`、`degrading` 判断。当前 aggregate-only 证据不能给单节点做三网排名；未来如需单节点评分，须另行设计保护隐私的 per-node evidence contract。
