...
def test_v1_and_v2_history_migrate_to_v3_without_changing_fingerprints() -> None:
    key = derive_fingerprint_key("token")
    ...
    for version in (1, 2):
        legacy = {"version": version, "nodes": {fingerprint: record}}
        if version == 2:
            legacy["cohort"] = {"runs": 2, "latency_ema_ms": 120.0, "last_seen_epoch": 100}
        history, status = parse_history_bytes(json.dumps(legacy).encode())
        assert status == "migrated"
        assert history["version"] == 3
        assert set(history["nodes"]) == {fingerprint}
        assert history["nodes"][fingerprint]["historically_preferred"] is True


def test_history_demotes_established_low_ema_node_without_recent_failures() -> None:
    key = derive_fingerprint_key("token")
    stable = {"good", "bad", "new"}
    history = {
        "version": 3,
...
def test_history_update_is_anonymous_and_persists_preference_decision() -> None:
    ...
    stable_record = updated["nodes"][fingerprint_runtime_name("stable-secret", key)]
    assert stable_record["historically_preferred"] is False
    records = list(updated["nodes"].values())
    assert sorted(record["success_ema"] for record in records) == [0.0, 0.666667, 1.0]
    assert updated["cohort"] == {"runs": 1, "latency_ema_ms": 180.0, "last_seen_epoch": 1000}
