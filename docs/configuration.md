...
`runtime.dns.mode: client` omits generated DNS state and leaves DNS behavior to the client environment. `managed` emits the declared DNS settings. Canonical production declares managed Fake-IP DNS with IPv6 DNS responses disabled, domestic IP-literal bootstrap resolvers, `respect-rules: false`, and a small LAN/local Fake-IP exclusion list, then lets `runtime.network_profile` decide the effective proxy-server resolver pool. DIRECT resolution includes the local system resolver and encrypted domestic fallbacks. `nameserver-policy` merges ACL4SSR rule-set routes with exact domain overrides, including `stun.l.google.com`; wildcard host patterns are rejected. Resolver transport does not recurse into proxy routing.
...
The `cn_three_net` URLTest tuning makes the client's own measurements decide node quality — no carrier identities are encoded anywhere. Regional country `url-test` groups (香港节点 / 台湾节点 / 新加坡节点 / 日本节点 / 韩国节点 / 美国节点) keep `interval: 300` and `timeout: 8000`, with switch tolerance widened to 120ms (US keeps 150ms) so carrier jitter cannot thrash selection. Browsing tiers keep `interval: 180` / `timeout: 8000`; `下载自动` stays low-frequency (600s) with high tolerance (300ms); every non-AI probe stays the canonical `https://cp.cloudflare.com/generate_204`, and browsing groups switch after one failed probe (`max-failed-times: 1`) while regional and other non-AI automatic groups retain `2`. The tuning pass fail-closes if scheduling declarations or compiled groups drift from this contract.
