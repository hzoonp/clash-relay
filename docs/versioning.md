...
`clash-relay` follows Semantic Versioning. `v0.1.0` established the first production baseline and `v1.0.0` froze the public compatibility contract. Later v1 minor releases may add compatible routing/runtime qualification capabilities while preserving source permissions and private-publication boundaries.
...
A minor release may add a compatible scenario surface, explicitly declared classifier extensions, probes, regional scheduling dimensions, safe observability, state behavior, or internal architecture contracts while preserving existing security boundaries.
...
- `ProxyLite.list` is the canonical generic foreign-web classifier and targets `网页浏览`. `ProxyGFWlist` is not the canonical browser classifier.
...
- Canonical production-specific selector names, compatibility members, AI display names/exclusions, ACL4SSR binding targets, and priority edges are declared under `routing.contract` in `policies.yaml`; core Routing V2 audit code consumes the declaration rather than maintaining a second hard-coded production policy.
...
- OpenAI, Claude, and Gemini qualify independently and fail closed per service. The canonical contract excludes Hong Kong before qualification. The private AI cache is payload-sensitive and TTL-bounded and falls back to live probing when reuse is unsafe.
- Media/messaging automatic routing remains transport-qualified while explicit/manual general node choices remain available.
- The exact final production candidate is validated with **every stable Mihomo core declared in `tools/mihomo-versions.json`** before publication. Workflow YAML and documentation must not create a second version list.
- A failed ACL fidelity, source reachability, qualification, current-policy audit, core-validation, or production-publication gate does not intentionally leave an unvalidated candidate active.
- Production activation stages immutable SHA-256 release objects and then updates the fixed client-facing Cloudflare KV key. Release pointer failure after activation invokes compensating restoration when a previous value exists; Workers KV is not falsely treated as a cross-key atomic store.
- Manual rollback requires explicit confirmation, resolves versioned previous-release state, applies the **current** source/Routing V2 policy audit, and revalidates with the full stable Mihomo matrix before activation.
- AI cache and scheduler history are derived optimization state. Their post-publication persistence may warn and retry on later runs without converting an already committed validated production release into a false deployment failure.
- Production configuration, immutable release bytes, previous-good bytes, scheduler state, AI cache, production metrics, subscription responses, and node-level qualification results are private operational data and are not attached to GitHub Releases.
...
clash-relay declarations
  -> source admission
  -> routing.contract
  -> source-aware inventories
  -> AI / Download declared extensions

clash-relay runtime pipeline
  -> RuntimeGraph audit
  -> live qualification
  -> regional/transport scheduling
  -> stable Mihomo matrix validation
  -> versioned private publication
```

## Persistent state compatibility

Persistent state must either be authoritative production-release metadata or safely degradable derived state.

- Versioned release IDs are SHA-256 hashes of exact production bytes. `current-release-v1` / `previous-release-v1` are private operational pointers; legacy `previous-v1` remains a migration fallback.
...
Release notes contain public source-code changes only. Generated production configuration, subscription data, scheduler state, AI cache, production metrics, previous/release bytes, Cloudflare KV values, and node-level qualification results are never release assets.
