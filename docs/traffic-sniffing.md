...
Traffic sniffing is an optional Mihomo runtime layer and does not change DNS ownership, routing policy, source isolation, or node qualification.
...
`runtime.sniffer` is optional. Projects that omit it keep sniffing disabled. Managed DNS and traffic sniffing can be enabled independently; neither feature implicitly enables the other.
...
Traffic sniffing does not change ACL4SSR rule order, BanAD, public proxy groups, browsing/AI qualification, regional Stable/Reserve scheduling, subscription permissions, multiplier filtering, EMBY filtering, publication policy, or the `subscription_1` browsing/AI-only boundary.
