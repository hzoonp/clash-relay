...
- `carrier_qualification` — an optional external self-hosted advisory boundary (`src/clash_relay/carrier_qualification.py`). The repository and GitHub-hosted Runner ship no carrier probes. Supply a JSON file outside `.work/private` with `scripts/run_production_release.py --carrier-qualification-input PATH`, or manually supply the same aggregate through the workflow dispatch input. The lifecycle validates it after cleanup and snapshots it privately. Accepted shape: `{"schema_version": 1, "collected_at_epoch": <epoch>, "carriers": {"telecom" | "unicom" | "mobile": {"tested", "reachable", "median_latency_ms"}}}`. The timestamp is required; future timestamps fail closed. Fresh evidence from one or two carriers is `partial`, all three carriers is `full`, evidence older than six hours is `stale`, and no input is `not_configured`. Actions, proof, and manifest expose only per-carrier aggregates and the aggregate tested/reachable/ratio, with no cross-carrier latency median. This advisory evidence never removes nodes, changes quality tiers or scheduling, or participates in Promotion Guard.
