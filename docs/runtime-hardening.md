...
- `carrier_qualification` — an optional external self-hosted advisory boundary. The independent carrier workflow probes on dedicated protected Telecom / Unicom / Mobile runners, merges only anonymous aggregates, and passes them to canonical preflight. Production regenerates the candidate and verifies the repository-bound sample, inventory, and probe-plan IDs before consuming evidence. Coverage is `not_configured`, `partial`, or `full`; freshness is `current` or `stale`; evidence quality is `sufficient`, `insufficient`, or `stale`. Actions, proof, and manifest expose aggregate-only counts and per-carrier latency, never raw targets. Carrier history persistence is two-phase: Phase A (prepare) runs inside a strictly read-only preflight; ordinary dry-runs issue no committable receipt — candidate binding, qualification validation, and the full production gate must succeed before a local privacy-safe observation receipt is issued binding the receipt schema version, aggregate digest, candidate/probe binding digest, collected epoch, and validated SHA; it contains no node, server, IP, hostname, source, credential, raw sample, or HMAC key material. Phase B (commit) is the separate `scripts/persist_carrier_observation.py` entrypoint, which accepts only such a receipt — never a bare carrier JSON — authenticates the repository-keyed receipt MAC and re-verifies every digest and the validated SHA fail closed, and is skipped entirely when a preflight fails. Publishing lifecycles only issue a validated receipt after the qualification/release gate; a separate post-release history commit owns the KV write and its failure cannot invalidate the verified config release. A separate versioned `carrier-observation-history-v1` KV state (document schema v3) stores only numeric campaign counts and per-carrier EMAs for candidate-bound campaigns; `binding_failed` is not a persistable status (statuses are `valid`/`stale`/`partial`/`insufficient`/`invalid`). `recent_campaign_count` is the trailing 30-day window (reaped on write and read); `status_counts_lifetime` and `campaign_runs_lifetime` are saturating lifetime counters capped at 64, not rolling values; `latency_sample_runs` and `last_latency_epoch` give latency EMAs explicit freshness so a zero-reachable campaign is never read as latency 0. The state never affects node removal, quality tiers, scheduling, or Promotion Guard. Aggregate-only evidence cannot support per-node carrier ranking.
