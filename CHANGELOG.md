...
- The qualification summary separates reachability evidence into `global_preflight_reachable` (GitHub Runner TCP endpoint admission), `client_runtime_health` (client-side URLTest contract plus browsing/AI qualification outcomes), and an optional pluggable `carrier_qualification` boundary for self-hosted Telecom / Unicom / Mobile probes that publishes aggregate-only results.
- Qualification now reports aggregate `node_quality_tiers` (robust / reserve / quarantined) per evidence unit: runner endpoint evidence (admitted on every attempt vs. 1-of-3 quorum vs. dead), runner hostname evidence, and browsing Stable/Reserve node evidence that maps directly onto the region-first automatic scheduling.
- Qualification now reports aggregate `removed_nodes` diagnostics (`by_source`, `by_region`, `by_protocol`, `by_failure_category`) and flags every subscription source whose entire runtime inventory was removed under `sources_fully_removed` with per-source failure-category reasons; no node identities are emitted.
- `carrier_qualification` accepts pluggable self-hosted aggregate payloads (`schema_version` 1 with per-carrier `tested`/`reachable`/`median_latency_ms` rows for telecom / unicom / mobile) from a lifecycle-private `carrier-qualification.json`; validation fails closed on identity-bearing or unknown fields and the report defaults to `not_configured`.
...
- Runner proxy hostname qualification probes DoH endpoints with standard RFC 8484 `application/dns-message` wireformat instead of the Cloudflare JSON API, so resolver-specific JSON API differences (AliDNS, DNSPod) cannot cause false "unresolvable" verdicts; the two-DoH-endpoint minimum is preserved.
- Runner TCP endpoint admission uses bounded retry with a 1-of-3 admission quorum: a transient timeout never permanently quarantines an endpoint on its own, and failures are classified (`dns_failure`, `connect_timeout`, `refused`, `network_unreachable`, `connect_failure`). Endpoints admitted with partial failures carry reserve evidence.
- Hostname qualification distinguishes DNS-confirmed unresolvable (quarantined) from runner transport failures (inconclusive — the node is kept, never mass-quarantined), and a stage where no resolver returned any DNS response fails closed instead of silently shipping or dropping the inventory.
